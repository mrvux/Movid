...
#include "moArithmeticModule.h"

MODULE_DECLARE(Arithmetic, "mrvux", "Performs per element operation on two images");

moArithmeticModule::moArithmeticModule() : moModule(MO_MODULE_INPUT|MO_MODULE_OUTPUT, 2, 1) {
	MODULE_INIT();
...
moArithmeticModule::~moArithmeticModule() 
{
...
void moArithmeticModule::notifyData(moDataStream *input) 
{
...
void moArithmeticModule::update() {
...
void moArithmeticModule::setInput(moDataStream *stream, int n) {
	assert( n == 0 || n == 1 );
...
moDataStream *moArithmeticModule::getInput(int n) {
	if ( n == 0 )
		return this->input1;
	if ( n == 1 )
		return this->input2;
...
moDataStream *moArithmeticModule::getOutput(int n) {
	if ( n != 0 ) {
